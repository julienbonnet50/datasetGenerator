...
object RandomDataGenerator {
  ...
  def forType(
    ...
    val valueGenerator: Option[() => Any] = dataType match {
      case StringType => Some(() => UUID.randomUUID().toString.substring(0, 10))
    ...
    // TODO :
    //   case dt @ DecimalType(_, _) =>
    //     val precision = rand.nextInt(19) + 1 // Generate a random precision between 1 and 19
    //     val scale = rand.nextInt(precision)  // Generate a random scale between 0 and precision - 1
    //     Some(
    //         () => BigDecimal.apply(
